...
public interface BasicAuthorizerCacheManager
{
   ...
   *
   * */
  default Optional<Policy.RowPolicy> getRowPolicyMap(
      String tableName,
  ...
  )
  {
    ...
    if (DEFAULT_ALL_ACCESS_USERS.contains(authenticationResult.getIdentity())) {
      return Optional.of(Policy.RowPolicy.ALLOW_ALL);
    }
    ...
    if (roleNames.contains("org-admin")) {
      return Optional.of(Policy.RowPolicy.createRestrictiveTenantIdPolicy(
          tenantColumnMatcher.group(1),
          fakeOrganization
      ));
    }
    return Optional.of(Policy.RowPolicy.createPermissivePolicy(FalseDimFilter.instance()));
  }
